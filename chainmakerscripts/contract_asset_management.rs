...
/// fn transfer_multi(json) // [{from, to, amount}, {from, to, amount},]
...
/// reference resources  https://my.oschina.net/u/3794778/blog/1808339
...
pub extern "C" fn init_contract() {
    ...
    if r.is_ok() {
        ...
        ctx.log(&format!("get state init#issued_amount is {:?}", issued_amount));
    }
    if issued_amount > total_supply {
        ctx.log("issued_amount more than total_supply");
...
/// transfer_multi 授权后多对多转账
///
/// @param json            [{from, to, amount}, {from, to, amount}]
/// @param json_arr.from   转账人地址，开户时返回地址，或者可查询地址
/// @param json_arr.to     收账人地址，开户时返回地址，或者可查询地址
/// @param json_arr.amount 金额，正整数
/// @ok "ok"
#[no_mangle]
pub extern "C" fn transfer_multi() {

    let ctx = &mut sim_context::get_sim_context();

    let pub_key = ctx.get_sender_pub_key();
    let address_my = calc_address(&pub_key);
    let param = ctx.arg_as_utf8_str("param");

    if param.len() == 0 {
        ctx.log("param is null");
        ctx.error("param is null");
        return;
    }

    let parsed = json::parse(param.as_str());
    if parsed.is_err(){
        let msg = format!("parse param to json fail. example json format: [{{\"from\", \"to\", 10}}], param is {}", param);
        ctx.log(&msg);
        ctx.error(&msg);
        return;
    }
    let tis = parsed.unwrap();
    if !tis.is_array() {
        let msg = format!("parse param to json fail. example json format: [{{\"from\", \"to\", 10}}], param is {}", param);
        ctx.log(&msg);
        ctx.error(&msg);
    ...
    let mut i = 0;
    while tis[i] != Null{
        let from = match tis[i]["from"].as_str(){
            Some(val) => val,
            None => {
                let msg = format!("param from[{}] is parse fail", tis[i]["from"]);
                ctx.log(&msg);
                ctx.error(&msg);
                return;
            },
        };
        let to = match tis[i]["to"].as_str(){
            Some(val) => val,
            None => {
                let msg = format!("param to[{}] is parse fail", tis[i]["to"]);
                ctx.log(&msg);
                ctx.error(&msg);
                return;
            },
        };
        let amount = match tis[i]["amount"].as_u32(){
            Some(val) => val,
            None => {
                let msg = format!("param amount[{}] is parse fail", tis[i]["amount"]);
                ctx.log(&msg);
                ctx.error(&msg);
                return;
            },
        ...
        let mut balance_allowance = 0;
        // 自己则不需要授权
        if from != address_my {
            balance_allowance = match get_allowance(ctx, &from, &address_my) {
                Ok(val) => val,
                Err(_) => {
