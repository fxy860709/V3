...
echo ${path}
pwd
num=$(sed -n '/tx_filter:/=' chainmaker.yml)
line_num=$(($num+3))
echo $line_num
echo "tx_filter 配置在第${num}行"
sed -i "$line_num s/type: \b[0-9]\+/type: 3/g" chainmaker.yml
#cat chainmaker.yml | tail -n +$num | head -n 10 |grep -v "# "

#num1=$(sed -n '/serialize type/=' chainmaker.yml)
#echo "snapshot 配置在第${num1}行"
line_num2=$(($num+15))
sed -i "$line_num2 s/type: \b[0-9]\+/type: 1/g" chainmaker.yml
cat chainmaker.yml | tail -n +$num | head -n 80 |grep -v "# "
